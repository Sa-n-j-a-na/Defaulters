    ...
    const fetchData = async () => {
      try {
        ...
        if (defaulterType === 'both' || defaulterType === 'latecomers') {
          const latecomersResponse = await fetch(`${process.env.REACT_APP_API_URL}/defaulterreport/${year}/latecomers/${fromDate}/${toDate}`);
        ...
        if (defaulterType === 'both' || defaulterType === 'dresscode') {
          const dresscodeResponse = await fetch(`${process.env.REACT_APP_API_URL}/defaulterreport/${year}/dresscode/${fromDate}/${toDate}`);
